...
from .datasets import get_dataset, LAMOLDataset
...
class TaskSequence(object):
    def __init__(self, args, task_keys, tokenizer, few_shot=False, lamol_format=False):
        ...
        self.balance_ratio = args.balance_ratio # [ALIREZA] this is for balancing the dataset

        # if any([x in PARLAI_TASKS for x in task_keys]):
        #     splits = ['train', 'test', 'dev']
        # elif any([x in CROSSFIT_QA_TRAIN_TASKS or x in CROSSFIT_CLS_TRAIN_TASKS for x in task_keys]) and (not args.task_collection or 'crossfit' in args.task_collection):
        #     splits = ['train', 'test', 'dev']
        # else:
        #     splits = ['train', 'test']
        splits = ['train', 'test', 'dev']
...
#                     positive_samples = sum([x[4] for x in self.encoded_dataset[task_key][split]])/len( self.encoded_dataset[task_key][split])
                ...
                if self.balance_ratio > 0 and split == 'train' and pos_ratio < self.balance_ratio:
                    logger.info(f"Using weighted random sampler for Task {task_key}, positive ratio ={pos_ratio}")
                    # 50/50
                    perfect_balance_weights = [1.0/(1-pos_ratio), 1.0/pos_ratio]
                    class_wieghts = [(1-self.balance_ratio)*perfect_balance_weights[0], self.balance_ratio*perfect_balance_weights[1]]
                    sample_weights = [class_wieghts[t] for t in data.targets]
                    w_sampler = WeightedRandomSampler(sample_weights, len(data.targets), replacement=True)
                    data_loader = DataLoader(data, batch_size=batch_size, sampler=w_sampler)
                else:
    ...
    def trim_subset(self, dataset: LAMOLDataset, n):
        ...
        if len(dataset.data) < n:
            logger.warning(f'{len(dataset.data)} < {n} when trimming dataset on {dataset.task_name}')
        else:
            # subset = random.sample(dataset.data, n)
            subset_data, subset_targets = zip(*random.sample(list(zip(dataset.data, dataset.targets)), n))
            dataset.data = subset_data
            dataset.targets = subset_targets
    
    def balance_dataset(self, dataset: LAMOLDataset, balance_ratio):
        #   d = [cq_input_ids, cq_input_masks, ans_input_ids, ans_input_masks, label]
        label2examples = defaultdict(list)
        for d in dataset.data:
            y=d[4]
            label2examples[y].append(d)
        
        min_examples_num = None
        min_label = None
        for label, examples in label2examples.items():
            if min_examples_num is None or len(examples) < min_examples_num:
                min_examples_num = len(examples)
                min_label = label
        ...
        for k, v in label2examples.items():
            if k == min_label:
                ret.extend(v[:min_examples_num])
            else:
                sample_size = round(min_examples_num * (1-balance_ratio) / balance_ratio)
                ret.extend(v[:sample_size])
        random.shuffle(ret)
        dataset.data = ret
        
    def sample_k_shot(self, dataset):
        #   d = [cq_input_ids, cq_input_masks, ans_input_ids, ans_input_masks, label]
        label2examples = defaultdict(list)
        for d in dataset.data:
            y=d[4]
