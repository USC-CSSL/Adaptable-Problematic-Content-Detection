from transformers.models.bart.modeling_bart import BartEncoder, BartDecoder, BartModel, \
    BartForConditionalGeneration, BartEncoderLayer, BartDecoderLayer, BartForSequenceClassification, BartClassificationHead
from transformers.models.bart.modeling_bart import shift_tokens_right, CrossEntropyLoss, Seq2SeqLMOutput, ACT2FN, Seq2SeqSequenceClassifierOutput
...
class BartForSequenceClassificationWithAdapter(BartForSequenceClassification, ModelWithAdapter):
    def __init__(self, config: BartConfig):
        super().__init__(config)
        ...
        if config.mtl:
            ...
            for task in config.tasks:
                self.classification_heads_dict[task] = BartClassificationHead(config.d_model,
                                                                              config.d_model,
                                                                              config.num_labels,
    ...
    def forward(
        ...
        eos_mask = input_ids.eq(self.config.sep_token_id)
...
class XisenBartForConditionalGenerationWithAdapter(BartForConditionalGeneration, ModelWithAdapter):
    def __init__(self, config: BartConfig):
        super().__init__(config)
