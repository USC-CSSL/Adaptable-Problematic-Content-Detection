...
def Linear(in_features, out_features, bias=True, initialize=True):
...
class SingleMLPWeightGenerator(nn.Module):
    def __init__(self, config, input_dim, output_dim, idx=None, prev_f_input_dim=None, initialize=True):
        super().__init__()
        self.config = config
        ...
        self.linear1 = lineary_factory(self.input_dim, self.hidden_dim, initialize=initialize)
        self.activation_fn = ACT2FN[config.activation_function]
        self.linear2 = lineary_factory(self.hidden_dim, self.output_dim, initialize=initialize)
...
class ParameterGenerator(nn.Module):
    def __init__(self, config, output_dims: List[int]):
        ...
        for idx, output_dim in enumerate(output_dims):
            if idx + 1 == len(output_dims):
                m = SingleMLPWeightGenerator(config, self.input_dim, output_dim, idx, prev_f_input_dim, initialize=False)
            else: 
