...
def save_predictions(config, gts, preds, probabilities, questions, tokenizer, filename):
    ...
    with open(os.path.join(config.output_dir, filename),'w') as wf:
        ...
        writer.writerows([(a,b,c,d) for a,b,c,d in zip(questions, gts, preds, probabilities)])
        wf.close()
        ...
        if args.no_param_gen: # directly train adapters
            optimizer_grouped_parameters = [
                ...
                {'params': [p for n, p in model.named_parameters() if any(nd in n for nd in no_decay)
                 ...
                 'weight_decay': 0.0},
                {'params': [p for n, p in model.named_parameters() if
                            not any(nd in n for nd in no_decay) and 'classification' in n],
                 'weight_decay': args.weight_decay},
                {'params': [p for n, p in model.named_parameters() if any(nd in n for nd in no_decay)
                            and 'classification' in n],
                 'weight_decay': 0.0}
            ]
        else:
            optimizer_grouped_parameters = [
